...
always @(posedge clk_i) begin
    if(rst_i) begin
        ...
        //低 9 bit 决定写当前 MR 里面的哪个地址 0-511
        ldmr_mrx_waddr <= mrx_waddr[MRX_ADDR_WTH-1 : 0];
        //写使能 只有当 DDR 数据握手成功时 ldmr_mrx_we = 1 时，说明这一拍 512-bit DDR 数据要写入 MR
        ldmr_mrx_we <= ldmr_ddrintf__rdata_vld_i && ldmr_ddrintf__rdata_rdy_o;
        //写哪类 MR”的选择信号打一拍
        ldmr_mrx_sel_dly1 <= ldmr_mrx_sel;
        //处理 跨 MR 地址边界 ,mrx_waddr[8:0]     ：当前 MR 内部地址,mrx_waddr[15:9]    ：跨过了几个 MR
        //全局 MR index = 起始 mrx_index + 地址高位进位
局部    // MR 地址 = 地址低 9 bit
